    ...
    fetch('/api/user')
        ...
        .then(user => {
            ...
            if (!user.isAdmin) {
                ...
                // 只有超级管理员（REDACTED_USER）才能看到成员管理和账号管理
                    ...
                    <tbody>
                        ${tasks.map(task => `
                            <tr>
                                ...
                                <td>
                                    <select onchange="updateTaskStatus(${task.id}, this.value)" class="task-status-select" ${user.username !== 'REDACTED_USER' ? 'disabled' : ''}>
                        ...
                        <tbody>
                            ${accounts.map(account => `
                                <tr>
                                    ...
                                    <td>
                                        ${account.username === 'REDACTED_USER' ? 
                                            '<span style="color: var(--text-secondary); font-size: 0.875rem;">超级管理员</span>' : 
