...
app.post('/api/login', async (req, res) => {
    ...
    if (login_type === 'password') {
        ...
        const emailCode = db.data.verification_codes.find(c => c.email === user.email && c.code === code);
...
app.post('/api/admin/rollback', requireSuperAdmin, async (req, res) => {
    ...
    try {
        ...
        const dbPath = path.join(__dirname, 'database.json');
        if (fs.existsSync(dbPath)) {
            const currentBackup = path.join(backupDir, `pre-rollback-${Date.now()}`);
            fs.mkdirSync(currentBackup, { recursive: true });
            fs.copyFileSync(dbPath, path.join(currentBackup, 'database.json'));
            res.json({ 
                success: true, 
                message: '已回滚到备份: ' + backupName,
                preBackup: currentBackup
            });
        }
...
app.use((req, res, next) => {
    ...
    if (writeMethods.includes(req.method) && apiPaths.some(p => req.path.startsWith(p))) {
        // 排除解锁和状态查询API
        const excludedPaths = ['/api/admin/db-unlock', '/api/admin/db-status', '/api/admin/db-lock', '/api/admin/backup', '/api/admin/backup-info'];
