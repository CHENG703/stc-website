...
const CMDLog = {
    ...
    executeCommand(command) {
        ...
        switch(cmd) {
            case 'help':
                ...
                this.log('rollback     - 回滚到指定备份', 'system');
                this.log('delbackup    - 删除指定备份', 'system');
            ...
            case 'rollback':
                if (!args) {
                    this.log('用法: rollback <备份名称>', 'error');
                    this.log('使用 backuplist 查看可用备份', 'info');
                    return;
                }
                if (!confirm('警告：回滚将覆盖当前数据库！是否继续？')) {
                    this.log('已取消回滚', 'system');
                    return;
                }
                this.log('正在回滚到备份: ' + args + '...', 'warn');
                fetchWithAuth('/api/admin/rollback', {
                    method: 'POST',
                    headers: {'Content-Type':'application/json'},
                    body: JSON.stringify({backupName: args})
                }).then(r=>r.json()).then(d=>{
                    if(d.success) {
                        this.log('回滚成功！', 'info');
                        if(d.preBackup) {
                            this.log('已自动备份当前数据到: ' + d.preBackup, 'warn');
                        }
                    } else {
                        this.log('回滚失败: ' + d.message, 'error');
                    }
                }).catch(e=>this.log('回滚失败: '+e.message,'error'));
                break;
            case 'delbackup':
                if (!args) {
                    this.log('用法: delbackup <备份名称>', 'error');
                    this.log('使用 backuplist 查看可用备份', 'info');
                    return;
                }
                if (!confirm('确定要删除备份 ' + args + ' 吗？此操作不可恢复！')) {
                    this.log('已取消删除', 'system');
                    return;
                }
                this.log('正在删除备份: ' + args + '...', 'warn');
                fetchWithAuth('/api/admin/backup/' + encodeURIComponent(args), {method: 'DELETE'}).then(r=>r.json()).then(d=>{
                    if(d.success) {
                        this.log('备份已删除: ' + args, 'info');
                    } else {
    ...
    connectToServerLogs() {
        // 使用EventSource连接服务器日志流
        if (typeof EventSource !== 'undefined') {
            try {
                this.eventSource = new EventSource('/api/logs/sse');
                ...
                this.eventSource.onerror = () => {
                    this.log('服务器日志连接断开，将在5秒后重连...', 'warn');
                    setTimeout(() => this.connectToServerLogs(), 5000);
                };
                this.log('已连接到服务器日志流', 'system');
            } catch (e) {
                this.log('浏览器不支持EventSource', 'error');
            }
        } else {
            this.log('浏览器不支持实时日志', 'warn');
        }
    }
};
